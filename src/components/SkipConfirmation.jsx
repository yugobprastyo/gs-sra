...
export default function SkipConfirmation({ isOpen, onClose, event, onConfirmSkip }) {
  ...
  // Jika currentInst adalah STACK, gunakan nextInst. Jika bukan STACK, gunakan currentInst.
  const targetLocation = String(currentInst).trim().toUpperCase() === 'STACK' ? nextInst : currentInst;
  ...
  return createPortal(
    <div className="fixed top-0 right-0 bottom-0 left-20 z-20 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      {/* Backdrop listener */}
      ...
      <div className="relative z-10 bg-white rounded-lg shadow-2xl w-full max-w-md h-fit max-h-[85vh] flex flex-col border border-gray-200 text-gray-800 font-sans overflow-hidden">
        ...
        <div className="p-5 space-y-3.5 text-xs overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            {/* INFO PENDUKUNG: Tampil jika user mencentang checkbox tetapi Next Instrument belum Ready */}
        ...
        <div className="flex items-center justify-end space-x-2 px-5 py-3 bg-gray-50 border-t border-gray-100 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="px-3.5 py-1.5 border border-gray-300 bg-white text-gray-700 hover:bg-gray-100 rounded font-semibold text-xs transition-colors cursor-pointer shadow-sm"
          >
            Cancel
          </button>
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded font-semibold text-xs transition-colors shadow-sm ${
              isSkipEnabled
